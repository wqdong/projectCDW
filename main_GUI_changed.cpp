...
int main(int argc, char** argv)
{
    ...
    int stable = 0;
    //color of cells 6*9
    vector<vector<int>> cell_colors(6, vector<int>(9, -1));
    vector<int> temp(9, -1);
    vector<int> prev;
    while(num < 6)
    {
        ...
        mean(frame);
        threshold( frame, dst, threshold_value, max_value, threshold_type );
...
//            cout<<num<<" capture a fig\n";
            ...
            prev = temp;
            temp = printFaceColor(frame, cells);
//            for (int i = 0; i < 4; i++) {
...
//                cout<<cells[i].x<<","<<cells[i].y<<endl;
            ...
            imshow("Rubik Recognition", frame);

        
        int sum_prev = accumulate(prev.begin(), prev.end(), 0);
        if (prev == temp) stable++;
        else if ( sum_prev != 0 && stable > 100 && cell_colors[prev[4]][4] == -1 )
        {
            stable = 0;
            cout<<"save a face\n";
            num++;
            vector<int> temp_ordered = prev;
            for(int i = 0; i < 9; i++)
            {
...
//                    }
                    ...
                    cell_colors[prev[4]] = temp_ordered;
